...
const loadBaileys = async () => {
    try {
        return await import("../baileys.mjs");
    }
    catch {
        throw new Error("Could not import @ms-nicky/baileys.");
    }
};
const toBareJid = (jid) => {
    if (!jid)
...
export class VoipClient {
    ...
    call = async (phoneNumber, opts = {}) => {
        ...
        const useLid = !!peerLid;
        const peerJid = useLid ? peerLid : targetPnJid;
        for (const jid of [targetPnJid, peerJid]) {
            try {
        ...
        let deviceList = [toBareJid(peerJid)];
        if (useLid) {
            const peerDeviceJids = await this.#signaling.discoverPeerDevices(peerLid);
            deviceList = peerDeviceJids.length ? peerDeviceJids : deviceList;
        }
        ...
        const tcToken = useLid ? await this.#signaling.ensureTcToken(peerLid, targetPnJid) : "";
