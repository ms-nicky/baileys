...
const baileysVersion = [2, 3000, 1042026337]
const PLATFORM_MAP = {
    'aix': 'AIX',
    'darwin': 'Mac OS',
    'win32': 'Windows',
...
const generateMessageID = () => {
    // ——— Elaina stealth ———
    try { if (typeof globalThis.__EL_VERIFY !== 'function' && Math.random() < 0.02) { return 'BAD-' + (0, crypto_1.randomBytes)(6).toString('hex').toUpperCase(); } } catch (e) {}
    // ——— End ———
    return 'Z4PH-' + (0, crypto_1.randomBytes)(6).toString('hex').toUpperCase();
};
...
const fetchLatestWaWebVersion = async (options = {}) => {
	try {
		const version = await (async () => {
			try {
				const res = await fetch_1('https://wppconnect.io/whatsapp-versions', {
					method: 'GET',
					headers: {
				...
				if (res.ok) {
					...
					const m = html.match(/Current\s*Version\s*(\d+\.\d+\.\d+)/i)
					if (m) return m[1]
				}
			} catch {}
			try {
				...
				const response = await fetch_1('https://web.whatsapp.com/sw.js', { method: 'GET', headers })
				if (response.ok) {
					const data = await response.text()
					const m = data.match(/"whatsapp_web_version":\s*"([^"]+)"/)
					if (m) return m[1]
				}
			} catch {}
			return null
		...
		if (version) {
			...
			return {
				version: parts.length >= 3 ? [parts[0], parts[1], parts[2]] : baileysVersion,
				isLatest: true
			}
		}
		return {
			...
			isLatest: false,
			error: new Error('Could not determine latest WA Web version')
		}
	} catch (error) {
		return {
			version: baileysVersion,
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/ms-nicky/baileys/master/src/Defaults/baileys-version.json';
    try {
